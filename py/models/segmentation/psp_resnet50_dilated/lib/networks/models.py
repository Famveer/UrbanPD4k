...
from ..nn import SynchronizedBatchNorm2d
BatchNorm2d = SynchronizedBatchNorm2d

class PSPNetImageProcessor:
    def __init__(self):
        # Pyramid module
        self.imgSizes = (300, 375, 450, 525, 600)  # Multi-scale sizes
        self.imgMaxSize = 1000  # Max input size
    ...
    def img_transform(self, img):
        ...
        img = self.normalize(torch.from_numpy(img.copy()))  # Normalize
        return img

    def round2nearest_multiple(self, x, p):
        """ Round x to the nearest multiple of p and ensure x' >= x """
        return ((x - 1) // p + 1) * p
    ...
    def __call__(self, img):
        ...
        for this_short_size in self.imgSizes:
            ...
            target_width = self.round2nearest_multiple(target_width, 8)
            target_height = self.round2nearest_multiple(target_height, 8)

            # Resize and transform image
            img_resized = img.resize((target_width, target_height), Image.NEAREST)
            img_resized = self.img_transform(img_resized)
            img_resized = torch.unsqueeze(img_resized, 0).to(self.device)

            img_resized_list.append(img_resized)

        return [x.contiguous() for x in img_resized_list]

    @classmethod
    def from_pretrained(cls, arch_model, device):
        print(f"Loading pretrained model: {arch_model}")
        cls.device = device
        return cls()  # ✅ Return an instance of the class

class PSPNetForSemanticSegmentation(nn.Module):
    def __init__(self):
        super(PSPNetForSemanticSegmentation, self).__init__()

    @classmethod
    def from_pretrained(cls, arch_model, encoder_path, decoder_path):
        ...
        return SegmentationModule(self.net_encoder, self.net_decoder, self.criterion)
...
class SegmentationModule(SegmentationModuleBase):
    ...
    def forward(self, x, *, segSize=None, seg_label=0):
        # training
        if segSize is None:
