...
class PlacePulse:
    ...
    def get_metrics(self, comparisons_df):
        return comparisons_df["category"].unique()
    
    def get_comparisons_count(self, comparisons_df):
        return comparisons_df["category"].value_counts()
    
    def get_voter_count(self, comparisons_df, metric="all"):
    
        user_dict = { k: f"user_{i}" for i,k in enumerate(comparisons_df["voter_uniqueid"].unique().tolist() )}
        comparisons_df["user_id"] = comparisons_df["voter_uniqueid"].apply(lambda x: user_dict[x])
        
        if metric.lower()!="all":
            return comparisons_df[comparisons_df["category"]==metric]["user_id"].value_counts()
            
        return comparisons_df["user_id"].value_counts()
        
    def get_evaluated_images(self, pp2_comparisons_df):
        left_id = pp2_comparisons_df["left_id"].unique()
        right_id = pp2_comparisons_df["right_id"].unique()
        total_img = set( set(left_id).union(set(right_id)) ) 
        
        return total_img

    def get_evaluated_images_per_category(self, pp2_comparisons_df):
        metrics = self.get_metrics(pp2_comparisons_df)
        m_dict = {}
        for metric in metrics:
            df_ = pp2_comparisons_df[pp2_comparisons_df["category"]==metric]
            left_id = df_["left_id"].unique()
            right_id = df_["right_id"].unique()
            total_img = set( set(left_id).union(set(right_id)) ) 
            m_dict[metric] = [len(total_img)]
        
        return pd.DataFrame(data=m_dict).transpose()
        
    ######################
    #  Mapping functions #
    ######################
        
    # Calculate locations for each sample
    def map_localization(self, df):
        City=[]
        Country=[]
        Continent=[]
        for Lat_long in df[["lat","long"]].values:
          for continent_obj in self.country_city_dataset:
            pol_cont = Polygon(continent_obj["gps"])
            inside_cont =  Point(Lat_long[0], Lat_long[1]).within(pol_cont)
            if inside_cont:
              for city_country in continent_obj['cities']:
                pol_count = Polygon(city_country["gps"])
                inside_count = Point(Lat_long[0], Lat_long[1]).within(pol_count)
                if inside_count:
    ...
    def map_match_result(self, choice):
        if choice=="lePr":
            return "left"
        elif choice=="rigPr":
            return "right"
    ...
    def map_category(self, metric):
        if metric=="safer":
            return "safety"
        elif metric=="livelier":
            return "lively"
        elif metric=="more boring":
            return "boring"
        elif metric=="wealthier":
            return "wealthy"
        elif metric=="more depressing":
            return "depressing"
        elif metric=="more beautiful":
            return "beautiful"
        else:
            return "none"

    country_city_dataset = [
                        {"continent": "South America",
                         "gps": [[13.685661, -56.590888],
                                [6.545604, -36.280288],
                                [-15.936026, -23.695735],
                                [-59.021794, -59.991849],
                                [-57.267919, -87.478106],
                        ...
                        {"continent": "North America",
                          "gps": [[55.296670, -46.965813],

                                 [13.685661, -56.590888],
                                 [2.760839, -121.880544],

                                 [59.779907, -178.251924],
                                 [67.321120, -167.897429],
                                 [75.932549, -162.865410],
                                 [73.854913, -70.768106]],
                         "cities":[
                              {
                              "country": "Mexico",
